    ...
    if direct_download_version!='na'
      ...
      script "Download Apache Tomcat #{direct_download_version}" do
        ...
        code <<-EOH
          ...
          chown -R #{tomcat_user} #{tomcat_install_loc}/tomcat
        EOH
      end
      ...
      if direct_download_version =~ /7(.*)/
        template "#{tomcat_install_loc}/tomcat/conf/server.xml" do
...
#convert version number to a string if it isn't already
      ...
      case tomcat_version
        when '6'
          ...
          template "#{tomcat_install_loc}/tomcat6/conf/server.xml" do
            source 'server6.xml.erb'
            owner "#{tomcat_user}"
            mode '0644'
          end
        ...
        when '7'
          ...
          script 'Download Apache Tomcat 7 ' do
            ...
            code <<-EOH
              ...
              chown -R #{tomcat_user} #{tomcat_install_loc}/tomcat7
            EOH
          end
          ...
          template "#{tomcat_install_loc}/tomcat7/conf/server.xml" do
            source 'server7.xml.erb'
            owner "#{tomcat_user}"
            mode '0644'
          end
